    ...
    for (int elt = 0; elt < batch_size; ++elt)
    {
        ...
        AT_DISPATCH_FLOATING_TYPES(
            input.scalar_type(), "trilinear_cpu_forward", ([&]
                ...
                TriLinearCPU2DSliceAndLUTTransformForward(
...
void TriLinearCPUSliceBackward(const int nthreads,
                               ...
                               scalar_t *grid_grad,
                               scalar_t *image_grad,
                               scalar_t *grid_wights_grad,
                               scalar_t *grid_bias_grad,
                               scalar_t *lut_grad,
                               scalar_t *lut_weights_grad,
                               scalar_t *lut_bias_grad,
                               const int grid_dim,
                               const int grid_shift,
                               const scalar_t grid_binsize,
                               const int lut_dim,
                               ...
                               const int grid_per_ch)
{
    for (int index = 0; index < nthreads; index++)
    {
        ...
        for (int i = 0; i < grid_per_ch; ++i)
        {
            grid_grad[id00_xy + grid_shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 0] * w00_xy;
            grid_grad[id10_xy + grid_shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 0] * w10_xy;
            grid_grad[id01_xy + grid_shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 0] * w01_xy;
            grid_grad[id11_xy + grid_shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 0] * w11_xy;

            grid_grad[id00_xr + grid_shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 1] * w00_xr;
            grid_grad[id10_xr + grid_shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 1] * w10_xr;
            grid_grad[id01_xr + grid_shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 1] * w01_xr;
            grid_grad[id11_xr + grid_shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 1] * w11_xr;

            grid_grad[id00_yr + grid_shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 2] * w00_yr;
            grid_grad[id10_yr + grid_shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 2] * w10_yr;
            grid_grad[id01_yr + grid_shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 2] * w01_yr;
            grid_grad[id11_yr + grid_shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o[0] * grid_weights[3 * (i + grid_per_ch * 0) + 2] * w11_yr;

            grid_grad[id00_xy + grid_shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 0] * w00_xy;
            grid_grad[id10_xy + grid_shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 0] * w10_xy;
            grid_grad[id01_xy + grid_shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 0] * w01_xy;
            grid_grad[id11_xy + grid_shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 0] * w11_xy;

            grid_grad[id00_xg + grid_shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 1] * w00_xg;
            grid_grad[id10_xg + grid_shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 1] * w10_xg;
            grid_grad[id01_xg + grid_shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 1] * w01_xg;
            grid_grad[id11_xg + grid_shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 1] * w11_xg;

            grid_grad[id00_yg + grid_shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 2] * w00_yg;
            grid_grad[id10_yg + grid_shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 2] * w10_yg;
            grid_grad[id01_yg + grid_shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 2] * w01_yg;
            grid_grad[id11_yg + grid_shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o[1] * grid_weights[3 * (i + grid_per_ch * 1) + 2] * w11_yg;

            grid_grad[id00_xy + grid_shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 0] * w00_xy;
            grid_grad[id10_xy + grid_shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 0] * w10_xy;
            grid_grad[id01_xy + grid_shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 0] * w01_xy;
            grid_grad[id11_xy + grid_shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 0] * w11_xy;

            grid_grad[id00_xb + grid_shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 1] * w00_xb;
            grid_grad[id10_xb + grid_shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 1] * w10_xb;
            grid_grad[id01_xb + grid_shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 1] * w01_xb;
            grid_grad[id11_xb + grid_shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 1] * w11_xb;

            grid_grad[id00_yb + grid_shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 2] * w00_yb;
            grid_grad[id10_yb + grid_shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 2] * w10_yb;
            grid_grad[id01_yb + grid_shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 2] * w01_yb;
            grid_grad[id11_yb + grid_shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o[2] * grid_weights[3 * (i + grid_per_ch * 2) + 2] * w11_yb;
            ...
            image_grad[index] += grad_d * 1 / grid_binsize;
            // g
            ...
            image_grad[index + height * width] += grad_d * 1 / grid_binsize;
            // b
            ...
            image_grad[index + height * width * 2] += grad_d * 1 / grid_binsize;
            ...
            grid_wights_grad[3 * (i + grid_per_ch * 0)] += grad_o[0] * out_xy;
            grid_wights_grad[3 * (i + grid_per_ch * 0) + 1] += grad_o[0] * out_x;
            grid_wights_grad[3 * (i + grid_per_ch * 0) + 2] += grad_o[0] * out_y;

            grid_bias_grad[(i + grid_per_ch * 0)] += grad_o[0];
            ...
            grid_wights_grad[3 * (i + grid_per_ch * 1)] += grad_o[1] * out_xy;
            grid_wights_grad[3 * (i + grid_per_ch * 1) + 1] += grad_o[1] * out_x;
            grid_wights_grad[3 * (i + grid_per_ch * 1) + 2] += grad_o[1] * out_y;

            grid_bias_grad[(i + grid_per_ch * 1)] += grad_o[1];
            ...
            grid_wights_grad[3 * (i + grid_per_ch * 2)] += grad_o[2] * out_xy;
            grid_wights_grad[3 * (i + grid_per_ch * 2) + 1] += grad_o[2] * out_x;
            grid_wights_grad[3 * (i + grid_per_ch * 2) + 2] += grad_o[2] * out_y;

            grid_bias_grad[(i + grid_per_ch * 2)] += grad_o[2];
        }
