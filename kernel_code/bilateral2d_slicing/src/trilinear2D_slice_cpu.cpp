...
template <typename scalar_t>
void TriLinear2DCPUSliceForward(const int nthreads,
                                const scalar_t *grid,
...
template <typename scalar_t>
void TriLinear2DCPUSliceBackward(const int nthreads,
                                 ...
                                 const int grid_per_ch)
{
    for (int index = 0; index < nthreads; index++)
    {
        ...
        for (int i = 0; i < grid_per_ch; ++i)
        {
            ...
            grid_grad[id00_xy + shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 0] * w00_xy;
            grid_grad[id10_xy + shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 0] * w10_xy;
            grid_grad[id01_xy + shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 0] * w01_xy;
            grid_grad[id11_xy + shift * (3 * (i + grid_per_ch * 0) + 0)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 0] * w11_xy;

            grid_grad[id00_xr + shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 1] * w00_xr;
            grid_grad[id10_xr + shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 1] * w10_xr;
            grid_grad[id01_xr + shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 1] * w01_xr;
            grid_grad[id11_xr + shift * (3 * (i + grid_per_ch * 0) + 1)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 1] * w11_xr;

            grid_grad[id00_yr + shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 2] * w00_yr;
            grid_grad[id10_yr + shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 2] * w10_yr;
            grid_grad[id01_yr + shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 2] * w01_yr;
            grid_grad[id11_yr + shift * (3 * (i + grid_per_ch * 0) + 2)] += grad_o_r * weights[3 * (i + grid_per_ch * 0) + 2] * w11_yr;

            grid_grad[id00_xy + shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 0] * w00_xy;
            grid_grad[id10_xy + shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 0] * w10_xy;
            grid_grad[id01_xy + shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 0] * w01_xy;
            grid_grad[id11_xy + shift * (3 * (i + grid_per_ch * 1) + 0)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 0] * w11_xy;

            grid_grad[id00_xg + shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 1] * w00_xg;
            grid_grad[id10_xg + shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 1] * w10_xg;
            grid_grad[id01_xg + shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 1] * w01_xg;
            grid_grad[id11_xg + shift * (3 * (i + grid_per_ch * 1) + 1)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 1] * w11_xg;

            grid_grad[id00_yg + shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 2] * w00_yg;
            grid_grad[id10_yg + shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 2] * w10_yg;
            grid_grad[id01_yg + shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 2] * w01_yg;
            grid_grad[id11_yg + shift * (3 * (i + grid_per_ch * 1) + 2)] += grad_o_g * weights[3 * (i + grid_per_ch * 1) + 2] * w11_yg;

            grid_grad[id00_xy + shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 0] * w00_xy;
            grid_grad[id10_xy + shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 0] * w10_xy;
            grid_grad[id01_xy + shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 0] * w01_xy;
            grid_grad[id11_xy + shift * (3 * (i + grid_per_ch * 2) + 0)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 0] * w11_xy;

            grid_grad[id00_xb + shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 1] * w00_xb;
            grid_grad[id10_xb + shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 1] * w10_xb;
            grid_grad[id01_xb + shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 1] * w01_xb;
            grid_grad[id11_xb + shift * (3 * (i + grid_per_ch * 2) + 1)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 1] * w11_xb;

            grid_grad[id00_yb + shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 2] * w00_yb;
            grid_grad[id10_yb + shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 2] * w10_yb;
            grid_grad[id01_yb + shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 2] * w01_yb;
            grid_grad[id11_yb + shift * (3 * (i + grid_per_ch * 2) + 2)] += grad_o_b * weights[3 * (i + grid_per_ch * 2) + 2] * w11_yb;
            ...
            wights_grad[3 * (i + grid_per_ch * 0)] += grad_o_r * out_xy;
            wights_grad[3 * (i + grid_per_ch * 0) + 1] += grad_o_r * out_x;
            wights_grad[3 * (i + grid_per_ch * 0) + 2] += grad_o_r * out_y;

            bias_grad[(i + grid_per_ch * 0)] += grad_o_r;
            ...
            wights_grad[3 * (i + grid_per_ch * 1)] += grad_o_g * out_xy;
            wights_grad[3 * (i + grid_per_ch * 1) + 1] += grad_o_g * out_x;
            wights_grad[3 * (i + grid_per_ch * 1) + 2] += grad_o_g * out_y;

            bias_grad[(i + grid_per_ch * 1)] += grad_o_g;
            ...
            wights_grad[3 * (i + grid_per_ch * 2)] += grad_o_b * out_xy;
            wights_grad[3 * (i + grid_per_ch * 2) + 1] += grad_o_b * out_x;
            wights_grad[3 * (i + grid_per_ch * 2) + 2] += grad_o_b * out_y;

            bias_grad[(i + grid_per_ch * 2)] += grad_o_b;
        }
    }
}

void TriLinear2DCPUSliceBackwardLaucher(const torch::Tensor &grad_output,
